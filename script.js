...
// Capsule Navigation Progress & Active States
function initCapsuleNavigation() {
  const navItems = document.querySelectorAll('.case-nav-pill .nav-item');
  const progressFill = document.getElementById('nav-progress');
  const sections = document.querySelectorAll('section[id], header[id]');

  if (!navItems.length || !progressFill || !sections.length) return;
  ...
  function updateNavigation() {
    ...
    // Update progress bar (7 sections = 100% / 7 ≈ 14.28% per section)
    const progress = Math.min((currentSectionIndex / (sections.length - 1)) * 100, 100);
...
// Initialize capsule navigation when DOM is ready
document.addEventListener('DOMContentLoaded', initCapsuleNavigation);